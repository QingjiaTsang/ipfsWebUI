...
class NetworkTraffic extends React.Component {
  ...
  componentDidUpdate(_, prevState) {
    ...
    console.log("nodeBandwidth", nodeBandwidth);
    const down = nodeBandwidth
      ? parseInt(nodeBandwidth.rateIn.toFixed(0), 10)
      : 0;
    const up = nodeBandwidth
      ? parseInt(nodeBandwidth.rateOut.toFixed(0), 10)
      : 0;

    if (
      down !== prevState.downSpeed.filled ||
      up !== prevState.upSpeed.filled
    ) {
      this.setState({
        downSpeed: {
          filled: down,
