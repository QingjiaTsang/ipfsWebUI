...
const WorldMap = ({ t, className, selectedPeers, doSetSelectedPeers }) => {
  ...
  const handleMapPinMouseEnter = useCallback(
    (peerIds, element) => {
      console.log("peerIds, element", peerIds, element);
      if (!element) return;
    ...
    <div className="flex flex-column">
      <div className={`relative ${className}`}>
        ...
        <div className="no-events absolute bottom-0 left-0 right-0">
          <div className="flex flex-auto flex-column items-center self-end pb5-ns no-select">
            ...
            <div className="f4 b ttu charcoal-muted">
              {t("app:terms.peers")}
            </div>
          </div>
        </div>
      ...
      <div className="relative flex justify-end pt2 pb4">
        <div className="f6 p2 no-select flex items-center">
          <span className="f6 charcoal-muted pr3">
            {t("app:terms.peers")}:{" "}
          </span>
          <i
...
const PeersCount = connect("selectPeers", ({ peers }) =>
  ...
  ({
    ...
    peersCoordinates.forEach(({ peerIds, coordinates }) => {
      el.append("path")
        ...
        .attr("class", "mapDot")
        .on("mouseenter", () =>
          handleMouseEnter(peerIds, d3.event.relatedTarget)
        )
...
const PeerInfo = connect(
  "selectPeerLocationsForSwarm",
  ({ ids, peerLocationsForSwarm: allPeers, t }) => {
      ...
      <div className="f6 flex flex-column justify-center">
        {peers
          ...
          .map((peer, index) => {
            ...
            return (
              <div
                className="pa2 f7 flex items-center monospace"
                key={peer.peerId}
              >
                <CountryFlag
                  className="mr1"
