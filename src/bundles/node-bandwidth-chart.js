...
function createNodeBandwidthChart(opts) {
  ...
  return {
    ...
    doUpdateNodeBandwidthChartData:
      ...
      ({ dispatch }) => {
        chartData = {
          in: chartData.in.concat({
            x: timestamp,
            y: parseInt(bw.rateIn.toFixed(0), 10),
          }),
          out: chartData.out.concat({
            x: timestamp,
            y: parseInt(bw.rateOut.toFixed(0) * -1, 10),
          }),
        };

        // 通过新数据的时间戳来减去一天的时间戳，来找到最近一天的数据，最近一天之前的数据都会被slice掉，即只存放着最近一天的数据
        const startIndex = chartData.in.findIndex(
          (d) => d.x >= timestamp - opts.windowSize
        );
        if (startIndex > 0) {
        ...
        dispatch({
          type: "NODE_BANDWIDTH_CHART_DATA_UPDATED",
          payload: { chartData },
        });
      },
    ...
    reactUpdateNodeBandwidthChartData: createSelector(
      ...
      (bw, lastSuccess, enabled, chartData) => {
        ...
        if (
          !chartData.in.length ||
          lastSuccess > chartData.in[chartData.in.length - 1].x
        ) {
          return {
