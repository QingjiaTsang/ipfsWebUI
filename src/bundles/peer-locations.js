...
function createPeersLocations(opts) {
  ...
  const bundle = createAsyncResourceBundle({
    ...
    getPromise: ({ store }) =>
      peerLocResolver.findLocations(
        store.selectAvailableGatewayUrl(),
        store.selectPeers()
      ),
  ...
  bundle.selectPeerLocationsForSwarm = createSelector(
    ...
    // 请解释下面的函数在干什么
    // 1. 从 peers 中获取 peerId
    // 2. 从 locations 中获取 peerId 对应的位置信息
    // 3. 将位置信息转换为字符串
    // 4. 获取位置信息中的国家代码
    // 5. 获取位置信息中的经纬度
    // 6. 解析 peer.addr
    // 7. 获取 peer.addr 的字符串形式
    // 8. 解析 peer.latency
    // 9. 获取 peer.direction
    // 10. 判断 peer.addr 是否为私有地址
    // 11. 判断 peer.addr 是否为附近地址
    // 12. 获取 peer.streams 中的协议
    // 13. 获取 peer.protocols 中的协议
    // 14. 将 peerId, location, flagCode, coordinates, connection, address, protocols, direction, latency, isPrivate, isNearby 作为对象返回
    (peers, locations = {}, bootstrapPeers, identity) =>
      peers &&
      peers.map((peer) => {
        console.log("peer098", peer);
  ...
  bundle.selectPeersCoordinates = createSelector(
    ...
    (peers) => {
      console.log("peers56", peers);
      if (!peers) return [];

      const aaa = peers.reduce((previous, { peerId, coordinates }) => {
        ...
        const previousCoordinates = previous.map((prev) => {
          ...
          const isCloseInXAxis =
            x - COORDINATES_RADIUS <= currentX &&
            x + COORDINATES_RADIUS >= currentX;
          const isCloseInYAxis =
            y - COORDINATES_RADIUS <= currentY &&
            y + COORDINATES_RADIUS >= currentY;
        ...
        // 当前peer的坐标如果跟上一个peer的坐标相近，就把当前peer的id添加到上一个peer的peerIds数组中，否则就把当前peer的id和坐标添加到previousCoordinates数组中
        // 在地图中的效果就是，如果两个peer的坐标相近，就会显示在同一个点上，否则就会显示在不同的点上
        // 最后，本函数遍历完了，返回的就是一个附带有坐标信息的peers数组，这样，就能提供给D3组件去消费并绘制地图了
...
// 请解释下面的函数在干什么
// 1. 判断 peer.addr 是否为私有地址
// 2. 判断 peer.addr 是否为附近地址
// 3. 返回一个对象，包含 isPrivate 和 isNearby 两个属性
// 4. isPrivate 表示 peer.addr 是否为私有地址
// 5. isNearby 表示 peer.addr 是否为附近地址
// 6. 附近地址的判断依据是：peer.addr 的 IP 地址和本机的公网 IP 地址是否在同一个网段
// 7. 本机的公网 IP 地址是通过 getPublicIP(identity) 函数获取的
// 8. getPublicIP(identity) 函数的实现是通过遍历 identity.addresses 数组，找到第一个非私有地址，然后返回该地址
const isPrivateAndNearby = async (maddr, identity) => {
  ...
  if (publicIP) {
    if (ip.isV4Format(addr.address)) {
      // 通过子网掩码和公网ip来判断ip是否在同一个网段，即是否nearby
...
class PeerLocationResolver {
      ...
      this.geoipLookupPromises.set(
        ...
        this.queue.add(async () => {
          try {
            ...
            // const data = await lookup("https://ipfs.io", ipv4Addr);
            // const data = await lookup("https://dweb.link", ipv4Addr);
            console.log("ipv4Addr", ipv4Addr);
            console.log("gatewayUrls", gatewayUrls);
  ...
  // 对peer进行排序，优先选择最近的peer
  // 解释下面的函数在干什么
  // 1. 优化peerSet
  // 2. 优化peerSet的方法是，如果pass小于3，就返回一个新的peerSet，否则返回原来的peerSet
  // 3. 新的peerSet是通过对原来的peerSet进行排序，然后取前10个，前100个，前200个
  // 4. 为什么要这样做呢？因为如果peerSet很大，那么每次都去请求ipfs.io，会很慢，所以这里做了一个优化
  // 5. 优化的方法是，先取前10个，然后取前100个，然后取前200个，这样就可以快速的更新地图
